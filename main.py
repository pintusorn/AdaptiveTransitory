...
MAX_BRAKE_STEP = 50
EMER_COUNT_LIMIT = 30
SAFE_DISTANCE_EXTRA = 4
SAFE_DISTANCE_MIN = 14
...
def record_prev_accel(prev_accel_dict, vehicle_id, accel_val):
    prev_accel_dict[vehicle_id] = accel_val

def get_prev_accel(prev_accel_dict, vehicle_id):
...
def main():
    ...
    gap = int(speed * headway) + SAFE_DISTANCE_EXTRA
    ...
    def get_controller_function(veh_id, controller, platoon, leader_id, headway):
        nonlocal int_gap_error, flag_merge_time, leader_mode, transitory_switch

        if controller == "dmpc": time_step = TIME_STEP_DMPC
        else: time_step = TIME_STEP_DEFAULT
        ...
        d_safe = max((headway * ego_speed) + SAFE_DISTANCE_EXTRA,14)
        ...
        if controller == "cacc":
            ...
            a_des = dmpc_controller(veh_num, ego_speed, ego_pos, pred_speed, pred_pos, pred_accel, leader_speed, leader_pos, leader_accel, d_safe, get_prev_accel(prev_accel_dict, leader_id), get_prev_accel(prev_accel_dict, f"p1veh{int(veh_id[-1]) - 1}"))
        ...
        log_vehicle_data(log_file, traci.simulation.getTime(), veh_id, ego_speed, ego_accel, get_prev_accel(prev_accel_dict, veh_id), d_gap, d_safe, ego_pos,leader_pos,d_safe_leader,most_leader_pos-ego_pos,leader_mode)
        ...
        for veh_id in platoon1:
            if veh_id in traci.vehicle.getIDList():
                ...
                if veh_id == "p1veh1":
                    ...
                    ego_accel = traci.vehicle.getAcceleration(veh_id)
                    ego_pos = traci.vehicle.getPosition(veh_id)[0]
                    log_vehicle_data(log_file, traci.simulation.getTime(), veh_id, ego_speed, ego_accel, get_prev_accel(prev_accel_dict, veh_id), 0, 0, ego_pos, ego_pos,0,0,leader_mode)
                    ...
                    if flag_disturbance == 1:
                        ...
                        record_prev_accel(prev_accel_dict, veh_id, traci.vehicle.getAcceleration(veh_id))
                    else:
                ...
                if veh_id_2 in traci.vehicle.getIDList():
                    ...
                    if veh_id_2 == "p2veh1": 
                        if flag_merge == 0:
                            ...
                            record_prev_accel(prev_accel_dict, veh_id_2, traci.vehicle.getAcceleration(veh_id_2))
                            ...
                            if flag_emer == "on":
                                ...
                                emer_count += 1
                                if emer_count == EMER_COUNT_LIMIT: 
                                    print("Heyy step at", step)
                                    flag_emer = "off"
                                    emer_count = 0
                            if((traci.vehicle.getPosition(platoon1[-1])[0]-traci.vehicle.getPosition("p2veh1")[0]) - (max((headway * ego_speed) + SAFE_DISTANCE_EXTRA,14)) < SAFE_DISTANCE_MIN) and (traci.vehicle.getSpeed(platoon1[-1]) - traci.vehicle.getSpeed(veh_id_2) < -1):
                               flag_emer = "on"
                            log_vehicle_data(log_file, traci.simulation.getTime(), veh_id_2, ego_speed, ego_accel, get_prev_accel(prev_accel_dict, veh_id_2), traci.vehicle.getPosition("p1veh1")[0] - traci.vehicle.getPosition(veh_id_2)[0], 0, ego_pos, ego_pos,0,0,leader_mode)
